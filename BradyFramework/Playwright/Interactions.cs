...
namespace SpecFlowPlaywright
{
    public class Interactions
    {
        ...
        {
            await (await _page).Locator(selector).ClearAsync();
        ...
        /// <returns></returns>
        public async Task PressKeyboardKey(string keyName)
        {
           switch(keyName)
            {
                case "Enter":
                    await (await _page).Keyboard.PressAsync(keyName);
                    break;
            }
        ...
        public async Task SetScreenSize() => await (await _page).SetViewportSizeAsync(1536, 864);
